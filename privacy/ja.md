...
当方は、保管庫の中身（写真・動画そのもの、ファイル名、アルバム名、保管件数）を収集・保有することは一切ありません。他方、本アプリは無料プランにおける広告表示のための SDK（2.3）、利用状況の記録とクラッシュ情報のための SDK（2.4）、およびご意見・お問い合わせの送信のための SDK（2.5）を利用しています。これらの提供事業者が各社の定めに従って取得する情報については、それぞれの項をご覧ください。購入の扱いは 2.2 をご覧ください。

### 2.2 サブスクリプション課金に関する情報

本アプリの課金は Apple の App内課金（In-App Purchase）で処理されます。ご購入の状態は、Apple が端末に渡す記録を iOS の仕組み（StoreKit）で読み取って判定しており、課金管理のための第三者サービスは利用していません。購入レシート・購入履歴が当方のサーバーや第三者のサーバーへ送信されることはありません。

購入代金の決済そのものは Apple（App Store）が処理します。当方が、利用者のクレジットカード番号等の決済手段の情報を受け取ることはありません。

なお、購入が完了したという事実（金額・商品名・レシートを含まない項目名のみ）は、2.4 の利用状況の記録として送信されます。
...
- 利用者が本アプリに保管した写真・動画および整理情報は、利用者の端末内（本アプリ専用の保存領域）に保存されます。当方のサーバーには保存されません。
...
- 本アプリを端末から削除（アンインストール）すると、端末内に保存された保管庫の中身は削除されます。ただし、iOS の仕様により保管庫の鍵情報はキーチェーンに残ることがあります。これは同じ端末に本アプリを入れ直した際の初回起動時に破棄されます。ただし、iCloud 同期をオンにしていた保管庫の暗号化コピーは、アンインストール後も利用者の iCloud に残ります（同じ Apple ID で再インストール・サインインした際に復元できるようにするためです）。削除の方法は 6. をご覧ください。
- 2.2 の購入に関する情報は、購入の管理・復元に必要な期間、Apple において保管されます。
...
- データの削除: アプリ内で個別に削除できるほか、本アプリを端末から削除することで、端末内の保管庫の中身を削除できます（キーチェーンに残る鍵情報の扱いは 5. をご覧ください）。
